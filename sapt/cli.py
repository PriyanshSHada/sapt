...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_install.add_argument("--dry-run", action="store_true", help="Preview only, no changes")
    p_install.add_argument("--source", choices=["apt", "snap", "flatpak", "github"],
    ...
    p_remove.add_argument("--clean", action="store_true",
                          help="Also remove configs and cache (purge)")
    ...
    p_history.add_argument("-n", "--count", type=int, default=20, help="Number of entries")
    p_history.add_argument("--verify", action="store_true", help="Verify audit log integrity")

    # audit
    p_audit = sub.add_parser("audit", help="Summarize and verify the audit log")
    add_local_json_flag(p_audit)
    p_audit.add_argument("-n", "--count", type=int, default=10, help="Recent entries to include")
    p_audit.add_argument("--entries", action="store_true", help="Include recent entries in the report")
    p_audit.add_argument("--cve", nargs="*", metavar="PACKAGE", help="Check packages against OSV")

    # completion
    p_completion = sub.add_parser("completion", help="Print shell completion script")
    p_completion.add_argument("shell", choices=["bash", "zsh", "fish"], help="Shell to generate for")
    ...
    p_diff.add_argument("-n", "--count", type=int, default=20, help="Number of history entries")

    # undo
    p_undo = sub.add_parser("undo", help="Reverse the latest package change")
    p_undo.add_argument("--dry-run", action="store_true", help="Preview only, no changes")
    p_undo.add_argument("-y", "--yes", action="store_true", help="Skip confirmation")

    # agent
    p_agent = sub.add_parser("agent", help="Recommend and install an APT toolkit for a goal")
    p_agent.add_argument("goal", nargs="+", help="The task you want to accomplish")
    p_agent.add_argument("--dry-run", action="store_true", help="Preview only, no changes")
    p_agent.add_argument("-y", "--yes", action="store_true", help="Skip confirmation prompts")
    ...
    cache_group.add_argument("--stats", action="store_true", help="Show cache statistics (default)")
    cache_group.add_argument("--clear", action="store_true", help="Delete all cached AI responses")
    ...
    p_alias.add_argument("package", nargs="?", help="APT package target when setting an alias")
    ...
    config_group.add_argument("--set-provider", action="store_true", help="Change AI provider")
    config_group.add_argument("--set-model", action="store_true", help="Change AI model")
    config_group.add_argument("--set-key", action="store_true", help="Update API key")
    config_group.add_argument("--set-endpoint", action="store_true", help="Change endpoint URL")
    config_group.add_argument("--set-budget", type=float, metavar="USD", help="Set monthly AI budget")
    config_group.add_argument("--set-call-cost", type=float, metavar="USD", help="Set estimated cost per AI call")
    config_group.add_argument("--usage", action="store_true", help="Show monthly AI usage")
    config_group.add_argument("--reset", action="store_true", help="Delete config, fresh setup")
